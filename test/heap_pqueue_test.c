...
{
	int key_param = 10;
	pqueue_ty *pqueue = HeapPQueueCreate(PQCmpObjs, (void *)&key_param);
...
void TestPQueueEnqueue(void)
{
	...
	pqueue_ty *pqueue = HeapPQueueCreate(PQCmpObjs, key_param);
	int status = -1; 
	
	status = HeapPQueueEnqueue(pqueue, &brittney);
...
void TestPQueueDequeue(void)
{
	...
	HeapPQueueEnqueue(pqueue, &brittney);
	HeapPQueueEnqueue(pqueue, &sponge_bob);
	HeapPQueueDequeue(pqueue);
	HeapPQueueDequeue(pqueue);
	
	if (1 == HeapPQueueIsEmpty(pqueue))
	{
		GREEN;
...
void TestPQueuePeek(void)
{
	...
	celebs_name = HeapPQueuePeek(pqueue);
...
{
	pqueue_ty *pqueue = CreatePQueue();
	HeapPQueueClear(pqueue);

	if (1 == HeapPQueueIsEmpty(pqueue) && 0 == HeapPQueueSize(pqueue))
	{
		GREEN;
...
void TestPQueueErase(void)
{
	...
	to_find = HeapPQueueErase(pqueue, AreNamesMatch, nameExists);
	...
	to_not_find = HeapPQueueErase(pqueue, AreNamesMatch, nameNotExsits);
	if (NULL == to_not_find)
	{
		GREEN;
...
static pqueue_ty *CreatePQueue(void)
{
	pqueue_ty *pqueue = HeapPQueueCreate(PQCmpObjs, OFFSETOF(celebs_ty, priority));
		
	HeapPQueueEnqueue(pqueue, &brittney);
	HeapPQueueEnqueue(pqueue, &sponge_bob);
	HeapPQueueEnqueue(pqueue, &james);
	HeapPQueueEnqueue(pqueue, &chan);
...
static void PrintPQueue(pqueue_ty *pqueue)
{
	...
	while (!HeapPQueueIsEmpty(pqueue))
	{
		celeb_name = (celebs_ty *)HeapPQueuePeek(pqueue);
		celeb_age = (celebs_ty *)HeapPQueuePeek(pqueue);
		celeb_priority = (celebs_ty *)HeapPQueuePeek(pqueue);
